...
def transfer(step):
    ...
    if step == 0:
        app.frame.start = tk.Button(
            app.frame,
            text="Transférer des photos",
            font=(info["police"], info["taillePolice"]),
            fg="blue",
            command=lambda: transfer(1),
        )
        app.frame.start.grid(
            row=0, column=0, rowspan="3", columnspan="3", sticky="nesw"
        )
    elif step == 1:
        try:
            app.frame.text = tk.Label(
                app.frame,
                text="Choisir le dossier de destination",
                font=(info["police"], info["taillePolice"]),
            )
            app.frame.text.grid(row=0, column=0, columnspan=3, sticky="nesw")
            app.frame.buttonAuto = tk.Button(
                app.frame,
                text="Choix auto par le logiciel",
                font=(info["police"], info["taillePolice"]),
                fg="blue",
                command=lambda: choixDest(),
            )
            app.frame.buttonAuto.grid(row=1, column=0)
            app.frame.buttonManu = tk.Button(
                app.frame,
                text="Choix Manuel",
                font=(info["police"], info["taillePolice"]),
                fg="blue",
                command=lambda: choixFenetre("fileDest", 2),
            )
            app.frame.buttonManu.grid(row=1, column=2)
            app.frame.buttonBefore = tk.Button(
                app.frame,
                text="Précédent",
                font=(info["police"], info["taillePolice"]),
                fg="red",
        ...
        try:
            if info["fileDest"] != "":
                app.frame.text = tk.Label(
                    app.frame,
                    text="Choisir le dossier où sont les photos",
                    font=(info["police"], info["taillePolice"]),
                )
                app.frame.text.grid(row=0, column=0, columnspan=3, sticky="nesw")
                app.frame.buttonAuto = tk.Button(
                ...
                app.frame.buttonManu = tk.Button(
                    ...
                    font=(info["police"], info["taillePolice"]),
                    fg="blue",
                    command=lambda: choixFenetre("fileSrc", 3),
                )
                app.frame.buttonManu.grid(row=1, column=2)
                app.frame.buttonBefore = tk.Button(
                    app.frame,
        ...
        app.frame.text = tk.Label(
            app.frame, text="OK", font=(info["police"], info["taillePolice"])
        )
        app.frame.text.grid(row=0, column=0, columnspan=3, sticky="nesw")
        app.frame.buttonNext = tk.Button(
            app.frame,
            text="Recommencer",
            font=(info["police"], info["taillePolice"]),
            fg="blue",
...
def choixSrc():
    ...
    if allLetter == 0:
        ...
        addToLog(
            "Il y a plusieurs appareils connectés, veuillez choisir le bon appareil",
            True,
            True,
        )
        time.sleep(2)
...
def chooseNameOfFile():
    ...
    try:
        ...
        app.frame.text = tk.Label(
            app.frame,
            text="Choisir le nom du dossier de destination correspondant à la visualisation",
            font=(info["police"], info["taillePolice"]),
        )
        app.frame.text.grid(row=0, column=0, columnspan=3, sticky="nesw")
        app.frame.fichierAppareil = tk.Entry(
            app.frame, width=20, font=(info["police"], info["taillePolice"])
        )
...
def copy():
    ...
    for filePhoto in allPhotos:
        ...
        try:
            ...
            app.preview.img.photo = img
            app.preview.img.grid(row=1, column=0, columnspan="2")
            app.preview.buttonNext1 = tk.Button(
                app.preview,
                text="Photo précédente",
                font=(info["police"], info["taillePolice"]),
                fg="green",
                command=lambda: viewerController(False),
            )
            app.preview.buttonNext1.grid(row=2, column=0)
            app.preview.buttonNext2 = tk.Button(
                app.preview,
                text="Photo suivante",
                font=(info["police"], info["taillePolice"]),
                fg="green",
                command=lambda: viewerController(True),
            )
        ...
        for element in listOfFile:
            ...
            newLinkToDest = (
                ...
                + "_"
                + newNameOfFile
                + "_{:00004d}".format(count)
                + extensionOfFile.lower()
            )
            # print('We are going to copy '+newLinkToSrc+' > '+newLinkToDest)
            try:
                addToLog(
                    "[Copy] '" + newLinkToSrc + "' -> '" + newLinkToDest + "'",
                    False,
                    False,
                )
...
def main():
    ...
    app.grid_columnconfigure(0, weight=1, minsize="1000")
    ...
    app.quitter = tk.Button(
        app, text="Quitter", fg="red", command=lambda: destroyTkinter()
    )
    ...
    def resource_path(ressource_path):
        try:
            base_path = sys._MEIPASS
        except Exception:
            base_path = os.path.dirname(os.path.abspath(__file__))
        return os.path.join(base_path, ressource_path)
    ...
    app.frameLog = tk.Frame(
        app, height=50, bg="blue", width=200, borderwidth=1, relief="ridge"
    )
    ...
    app.frameLog.logtTXT = tkinter.scrolledtext.ScrolledText(
        app.frameLog, font=(info["police"], 10), bg="white"
    )
